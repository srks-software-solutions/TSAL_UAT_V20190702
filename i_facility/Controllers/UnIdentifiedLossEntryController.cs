    ...
    public class UnIdentifiedLossEntryController : Controller
    {
        ...
        public ActionResult Index(IList<UnIdentifiedLossCorrection> UnIdentifiedL)
        {
            ...
            try
            {
                ...
                {
                    DataTable LossCodesData = new DataTable();
                    string query = @"SELECT Distinct(HMIID) FROM i_facility_tsal.dbo.tblworeport WHERE MachineID IN ( " + MacIDs + ") and CorrectedDate = '" + CorrectedDate + "';";
                    using (MsqlConnection mcLossCodes = new MsqlConnection())
                    {
                        mcLossCodes.open();
                        SqlDataAdapter daLossCodesData = new SqlDataAdapter(query, mcLossCodes.msqlConnection);
                    ...
                    mcDeleteRows.open();
                    SqlCommand cmdDeleteWOData = new SqlCommand("DELETE FROM i_facility_tsal.dbo.tblworeport WHERE MachineID IN ( " + MacIDs + ") and CorrectedDate = '" + CorrectedDate + "';", mcDeleteRows.msqlConnection);
                    cmdDeleteWOData.ExecuteNonQuery();

                    for (int l = 0; l < LossCodesData.Rows.Count; l++)
                    {
                        SqlCommand cmdDeleteWOLosses = new SqlCommand("DELETE FROM i_facility_tsal.dbo.tblwolossess WHERE HMIID = '" + LossCodesData.Rows[l][0] + "' ", mcDeleteRows.msqlConnection);
                ...
                try
                {
                    SqlCommand cmdDeleteOEEData = new SqlCommand("DELETE FROM i_facility_tsal.dbo.tbloeedashboardvariables WHERE WCID IN ( " + MacIDs + ") and StartDate = '" + StartDate + "';", mcDeleteRows.msqlConnection);
